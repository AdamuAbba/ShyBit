...
describe("generate the redeem script", () => {
  test("generate the redeem script in hex format for the given preimage. Note: redeem_script => OP_SHA256 <lock_hex> OP_EQUAL", () => {
    const { redeemScriptHex } = redeemScriptHexGenerator(preimage);
    expect(redeemScriptHexGenerator(preimage)).toHaveProperty("p2shAddress");
    assert.strictEqual(redeemScriptHex, expectedRedeemScriptHex);
  });
});
